...
class Schedule:
    ...
    def initialize(self):
        print(
            "[*] Read excel file {} done. Start initialization.".format(self.excelPath)
        )
        self.checkSchTime()
        self.testSch()
        self.parseCells()
    ...
    def checkSchTime(self):
        ...
        self.schedule.insert(
            self.schedule.shape[1], "time_intervals", [None] + time_intervals
        )
    ...
    def getTodaySch(self, weekday):
        ...
        self.todaySch = self.todaySch.drop(
            [0]
        )  # drop notice, don't use inplace parameter because of some rules
    ...
    def parseCells(self):
        ...
        while index < self.todaySch.shape[0]:
            ...
            cellMode = (
                cellLines[0][1:-1]
                if cellLines[0].strip() in ["[multi]", "[section]"]
                else None
            )
            if cellMode == None:
                pass
                ...
                for line in cellLines:
                    ...
                    if line[0] == "[" and line[-1] == "]":
                        ...
                        if (
                            re.match(r"(\d{2}[.:]\d{2})-(\d{2}[.:]\d{2})", line[:11])
                            == None
                        ):
                            raise Exception(
                                "the format of the cell contains an error:\n{}".format(
                                    cell
                                )
                            )

                        timeInterval = scheduleTime.TimeInterval(
                            line[:11]
                        ).time_interval
                        newSchduleContent = line[11:].strip()
                        newSchdule = pd.DataFrame(
                            [[timeInterval, newSchduleContent]],
                            columns=["time_intervals", self.weekday],
                        )
                        self.addSchedule(index, newSchdule, mode=addScheduleMode)
                        if addScheduleMode == -1:
                            indexOverPlus = 1
                        addScheduleMode = (
                            -1 if addScheduleMode == 1 else -1
                        )  # only change once
                        index += 1
                index -= indexOverPlus
                ...
                if index == self.todaySch.shape[0] - 1:
                    raise Exception(
                        "the [section] label couldn't be replaced in the last schdule"
                    )
                for line in cellLines:
                    line = line.strip()
                    if line[0] == "[" and line[-1] == "]":
            ...
            index += 1
        scheduleTime.checkCorrect(
            list(self.todaySch["time_intervals"])
    ...
    def querySch(self, timeStr):
        ...
        def between(time, schIndex):
            ...
            if (
                time >= time_interval[0] + timedelta(seconds=-self.advancedRdTime)
                and time < time_interval[0]
            ):
                return True
            else:
                return False
